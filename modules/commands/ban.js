module.exports.config = {
	...
	description: "حظر العضو بشكل دائم من المجموعة",
  usePrefix: true,
...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage('يحتاج البوت الى ان يكون مسؤلا في المجموعة يرجى اضافته والمحاولة مرة اخرى!', threadID, messageID);
  ...
  */
  ...
  if(args[0] == "view") {
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Right cunt border!', threadID, messageID);
  	...
  	if(!id) return api.sendMessage("❎يجب إدخال معرف الشخص ليتم إزالته من قائمة الحظر في المجموعة", threadID, messageID);
  	bans.banned;
  	if(!mybox.includes(id)) return api.sendMessage("✅لم يتم حظر هذا الشخص من المجموعة بعد", threadID, messageID);
			api.sendMessage(`✅تمت ازالة العضو ${id} من قائمة الحظر بالمجموعة`, threadID, messageID);
  	...
  	msg == "" ? api.sendMessage("✅لم يتم حظر أي شخص في مجموعتك حتى الآن", threadID, messageID) : api.sendMessage("❎الأعضاء الذين تم حظرهم من المجموعة:\n"+msg, threadID, messageID);
  }
